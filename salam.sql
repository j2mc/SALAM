...
CREATE TABLE `hosts` (`id` int PRIMARY KEY AUTO_INCREMENT NOT NULL, `name` tinytext, `description` tinytext,`enabled` BOOLEAN, `site_id` int,`status` int, `avg_rt` bigint NOT NULL DEFAULT 0, `avg_count` int NOT NULL DEFAULT 0, `lastcheck` int, `uptime` int DEFAULT 0, `downtime` int DEFAULT 0, `warntime` int DEFAULT 0, `last_rt` int);
...
CREATE TABLE `services` (`id` int PRIMARY KEY AUTO_INCREMENT NOT NULL, `host_id` int, `type` tinytext, `port` tinytext, `name` tinytext, `enabled` BOOLEAN, `status` int, `avg_rt` bigint DEFAULT 0, `avg_count` int DEFAULT 0, `lastcheck` int DEFAULT 0, `uptime` int DEFAULT 0, `downtime` int DEFAULT 0, `warntime` int DEFAULT 0, `last_rt` int);
...
CREATE TABLE `archivedata` ( `id` INT NOT NULL AUTO_INCREMENT , `type` TINYTEXT NOT NULL , `type_id` INT NOT NULL , `time` INT NOT NULL , `min_rt` INT NOT NULL , `avg_rt` INT NOT NULL , `max_rt` INT NOT NULL , PRIMARY KEY (`id`));